...
function addCheckboxToFastActionUpdate() {
  ...
  var label = document.querySelector('.UpdateMyProfile .updateMyProfileSection label');
...
function generateAccordions(accordion_options) {
  var accordions_node = document.createElement('div');
  accordions_node.classList.add('accordions');
  var accordions = new Array();

  for (var i = 0; i < accordion_options.length; i++) {
  ...
  if (accordions.length) {
    for (var _i = 0; _i < accordions.length; _i++) {
      accordions_node.appendChild(accordions[_i]);
    }
...
function displayAccordion() {
  var top_accordions_node = generateAccordions(getThemeOption('top_accordions'));

  if (top_accordions_node) {
    var header = document.querySelector('header.HeaderHtml');
  ...
  var bottom_accordions_node = generateAccordions(getThemeOption('bottom_accordions'));

  if (bottom_accordions_node) {
    var footer = document.querySelector('fieldset.PaymentMethodSection');
...
function bgImage() {
  ...
  if (mainImage) {
    ...
    var attribution = generateAttribution(getThemeOption('main_image_attribution'));

    if (attribution) {
      _bgImage.appendChild(attribution);
    }
  }
}

function mobileImage() {
  var hero = document.querySelector('.responsive-hero');
